export const SPECIAL_WORDS = [
    '\n\n\n\n后来我才知道\n人类的悲喜并不相通',
    '\n也许当我死后\n世界才会开始爱我\n也许拥有过的\n终究会被一笔带过\n也许永远没人知道\n我的秘密\n也许我们总有一天\n都会离去',
    '\n\n\n\n我的小狗\n你也会有忧愁吗'
]

export const WORDS = [
    '\n\n\n\n      拜托拜托啦',
    '\n\n\n大反派的身份\n藏不住了',
    '\n\n哼！\n\n瞧不起谁呢\n展示~',
    '\n\n\n天黑请闭眼\n煜言家请睁眼',
    '\n\n\n我会鼠的\n真的会鼠的\n真的',
    '\n\n那我们一起\n喜欢\n每一个当下\n好吗？',
    '\n\n不管我们\n\n都来自哪里\n一起往前走叭～',
    '\n\n如果可以的话\n\n请 让我\n讲个祖传的笑话',
    '\n\n只要有一个人\n\n得到治愈\n这首歌就没有白唱',
    '\n\n歪哥：煜会唱吗\n(十年人间)？\n\n煜：我可以现在会',
    '\n希望你们永远不死\n永远不死太难的话\n\n那\n浅予深深\n长乐未央',
    '\n\n今天吃了一个桃\n爱上戴宝无处可逃\n\n（不对好像俺十二年前\n就吃过了)',
    '\n祝大家都有健康身体\n美好心情\n月不为空\n\n那希望我们都享受\n平淡日子里星辰',
    '\n\n歪哥：煜怎么上来了\n歪哥：赶我走？\n\n煜：歪哥\n我可以\n煜：跟你坐在一起吗',
    '\n坐石头台阶上\n只有月亮经过\n\n击鼓之后\n把在黑暗中跳舞的\n心脏叫作月亮\n这月亮由我们构成',
    '\n十年前那个吃着\n小熊饼干\n看着lol第一视角的我\n\n根本没想过\n十年后\n我的歌会成为～～～',
    '原来平行时空里\n早在那年在电脑前\n看godlie的时候\n就定下了今天的约定\n\n"你好\n早在我们遇见之前\n很久我就认识你咯"',
    '浩荡星河\n细小尘埃不断碰撞\n我相信人与人之间\n有磁场\n相信缘分\n相信某些奇妙际遇\n辽阔的世界\n因为某一些落脚处让\n散落的烁尘汇聚起来',
    '所以啊我就说\n人生之所以珍贵\n说不定\n不是我们去过的\n一个又一个\n精彩华丽的旅游胜地\n而是旅途之中 \n寂寞 无聊 不可打发的\n时间里\n有你陪伴吧',
    '人与人的链接\n不同的情绪\n有仪式感的时刻\n值得被纪念的瞬间\n我眼里的你们\n你们眼里的我\n稀松平常的每一块拼图\n和平淡岁月里星辰\n人总是对未来\n抱有美好期待的',
    '\n\n\n  甜只是我的保护色',
    '\n\n\n爱不能看嘴说的\n要看行动',
    '\n\n\n自信是一个人类\n最好的医美',
    '\n\n\n放下助人情结\n尊重他人命运',
    '\n\n\n以后不要轻易相信女人\n说的话',
    '\n\n抽烟只是把偷走的\n快乐\n\n还给你',
    '\n\n\n别管一个女生怎么夸\n你, 说走就走辣',
    '\n\n\n   人间不值得\n          开心最重要',
    '\n\n我做人一个原则\n\n不入他人因果\n不断他人财路',
    '\n\n加入我比比芭比啵比\n专业情感修复团队\n\n走向辉煌',
    '\n\n没有人需要对\n另外一个人买单\n\n每个人都要\n为自己买单',
    '\n\n世界不该只是变成\n你想要的样子\n他们只是在成为\n他们本来的样子',
    '\n\n没有谁是救世主的\n\n所以不要给自己\n太加那些\n不应该 你承受的包袱',
    '\n任何时候要有边界感\n\n要会拒绝\n要会say no\n不要什么东西都\n大包大揽',
    '\n放下偶尔冒出的\n优越感和排他性\n放下偏见与片面\n去看\n去感受\n去探索\n越拥抱世界\n也越拥抱自己',
    '\n没有期待就没有失望\n没有失望就没有痛苦\n\n可是\n问题在于\n如果没有失望\n没有痛苦 只有快乐\n就等于不快乐',
    '\n吸引力法则\n就是讲什么呢?\n\n就是你想什么\n就会吸引什么发生。\n如果你一直担心\n你担心的事情\n就可能会发生',
    '分开不一定是坏事\n每个人都有自己的人生\n你总会一些人\n在这段时间来了又走了\n下一段时间\n又会有新的人\n\n所以追求永远\n不如把现在过好',
    '如果你不接受失败\n你怎么会有\n下一次成功呢?\n马斯克曾经说过\n一件事情你想做成\n你至少要做10次。\n就是一件事你一定要\n做好你会失败十次的\n准备  你再去做',
    '自信就是\n你相信你能够\n做到你想做到的事情\n你相信你值得\n世界上所有的美好\n你相信哪怕你可以输\n可以失败 可以犯错\n但是任何一个节点都\n有从头再来的机会\n这是自信',
    '人难的就是控制\n自由自在是最简单的\n你必须要限制自己的一\n些情绪 行为 胡思乱想\n这个比较难。\n但如果一直被情绪\n牵着走 会很累 很痛苦\n所以你要想办法让\n自己过得轻松一点'
]

export const SPECIAL_LYRIC = [
    '摇四季轮回 我们都变老',
    '爱是自由意志地沉沦',
    '没有人再逼迫善良一定要战胜邪恶',
    '真实没有那么重要 重要的是真实感',
    '落日和你 星辰和你 都合理',
    '除了你什么都没失去 和往常一样 但和你一起失去的是我自己',
    '每个阶段都有终点站 我们得下车 走不同的路未来某个节点再会和',
    '一路上经过学校花店 荒野到海边 城堡和花园 还是怀念',
    '老人与海 美女与野兽 盛放的火焰我自己来庇护',
]

export const NEW_SONG_LYRIC = [
    '日落的黄昏正打在我脸上 我是个artist 在忘我地演唱',
    '不知觉情绪在湿润我眼眶 遗忘是另一种点亮',
    '美可以被伪造 可以被仰望 也可以被毁掉',
    '有钱能使鬼推磨 万能的药 那怎么不见效',
    '老人与海 美女与野兽 盛放的火焰我自己来庇护',
    '不好的让你找到什么不想要',
    '扮演过想要成为的样子 没把自己弄丢 也不算多潇洒',
    '曾经他问我经历的痛苦真的会有意义吗',
    '分离的那夜 南飞的大雁 背过身握住的挂念',
    '或许一转眼就沉溺 或许会留下点痕迹',
    '说一个瞒天过海的谎言 差点把自己都骗到',
    '幸运的是 我依然没被击垮 在自卑和自信间徘徊的我还在用力的击打',
]

export const LYRIC = [
    ...SPECIAL_LYRIC,
    ...NEW_SONG_LYRIC,
    '舍不得凶你 玫瑰花送你 有你 我心装不下别人变得拥挤',
    '没有办法 拿你没有办法 告诉妈妈没办法不看她',
    '没有办法 拿你没有办法 天使守护公主 像写童话',
    '爱是最甜的糖果',
    '惊喜来自未知才算作旅途 什么算不虚度',
    '闭上眼 看不见 想太多 tell me what u want',
    '睡不着 找不到 想太多 tell me what u want',
    '我会遵守时空顺序 找到你再不想失去',
    '我们穿过烟雾 不可能延误 看不懂的以为我们修炼仙术',
    '稀缺元素 入选年度 做到再回来宣布',
    '时常更新思想 让系统重启',
    '有多少悔恨当初没有勇气奋不顾身',
    '有多少年轻灵魂心想不能一事无成',
    '假装没遗憾 当死神带走我一半',
    '可人都有七情六欲 所以经历相同悲欢',
    '现在日子不好不坏活在黑与白的中间',
    '没改变这世界 也不确定是否被改变',
    'ohoh 普通的人生 普通的想法在脑海里发芽生根',
    'ohoh 普通的人生 普通的故事 第三第二或第一人称',
    '把自己打破再重塑的过程叫做长大',
    '阳关道 独木桥 最后各有各的遗憾',
    '完美的人只出现在概念里',
    '我想成熟的爱是爱具体的人',
    '爱包裹的心脏血肉模糊 到底是另外一种下放还是救赎',
    '好像权衡利弊就可以削弱回忆的威力',
    '定义为再也不会有 或坏结局其实也未必',
    '不要着急记录 等故事结束再尝试叙述',
    '往前走了好久 发现其实都在身后',
    '成为你想要成为的 别让他们把你变成谁',
    '我丢掉了车票 原来她才是我通往开心的车票',
    '拥有的和即将拥有让我不断努力',
    '是谁透过迷雾看到真相 佩戴警徽当上警长的感觉真的很棒',
    '不同选择决定不同的以后 消散的念头是缤纷的平行宇宙',
    '每个存在都值得被爱 值得被用心对待',
    '如果时间有尽头 你会不会在那里等我 还是注定要错过',
    '能解决 早解决了 没解决的 没解决呢',
    '好吧 明天又是新的一天 先把烦恼放一边',
    '慢一点慢一点 反正我们又不用赶时间',
    '替我许个愿望吧 让时间 美好 和爱 它们统统都来追我',
    '“小鱼会有自己的海洋 你有属于自己的模样”',
    '别总用理性标榜爱 用爱来试探人性',
    '不能选择出生 但可以一直出声 舞刀弄剑或是文人书生',
    '让美好属于世界 让我属于我',
    '平等地拥抱每个灵魂的橘红色日落 和清晨的第一股花香 我想那都是我',
    '我想要没有饥饿 没有寒冷 没有痛 没有穷 没有战争 没有残忍',
    '我想要目睹花开 听到兩落 吹到风 摸到雪 大地对蓝天许诺',
    '今天对明天来说也是回不去的从前',
    '人与人刚认识的时候温度刚好 越想深入 越需要双向奔赴',
    '我可以相信你但不相信人性 不相信白驹 我不相信概率',
    '没有办法不看她 她能懂我的反差',
    '你听到这首歌会感觉像朋友一样吗',
    '我要你不为人知的反面',
    '爱总会在它想来的时候到来',
    '如果没有你 其他人在我心里都是套牌',
    '爱偶尔让你丢失方位 太投入收获的幸福痛苦最后都是双倍',
    '你爱他肉体还是灵魂 爱他骨子里的浑浊还是只是表面清纯',
    '嘿 你好 好久没见',
    '记得 是为了向上帝表达抗议 哪怕我抗拒的命运是不可抗力',
    '我最害怕 时间让我忘了你的姓名',
    '等到你想要牵着我的手 我带你去环游 一起仰望宇宙',
    '你不需要变更优秀 因为我从没拿你和谁比较',
    '我希望棒棒糖还是你开心的理由',
    '在周末放个音乐庆祝 庆祝没放弃就没输',
    '成长的感触 包容或袒护 时间轴上不断赶路',
    '有人告诉他木秀于林 风必摧之 告诉他知之为知之 不知为不知',
    '对不起 把你变成你不喜欢的你',
    '对不起 没遵守承诺做到永远保护你',
    '对不起 用坏的态度对待你',
    '注入新的血液 留下改不掉的习惯',
    '我尝试控制梦境 却无法阻止现实有人敲门有人离开',
    '我也想过算了 还没转过弯呢',
    '人们为达到目的 早已经放弃真理 我不想跟他们一样骗自己',
    '不管你来自哪里 我们就是这个秋天最热门的话题',
    '黑夜与白昼的争斗 思维清晰舌战群雄的能手',
    '没人会被偏袒没实力只能遣返',
    '不能什么都想要 最差结果要想到',
    '如果你什么都想要 最后可能两手空空 什么都没得到',
    '找自己 别跟随着人群都前赴后继',
    '万物皆平等 脆弱时候需要疼爱',
    '大自然的生存法则 从来都在混沌中黑白夹杂着',
    '生命是条单向延展的线 视角多宽就有多少看得见',
    '真的才华不会被“按时分配” 成功之前 都会被暂时分类',
    '舒适圈的安逸是个陷阱 进入上帝视角才能把我唤醒',
    '只有我才能对我自己下令 从小我就相信我策划的蝴蝶效应',
    '就让我在故事里安睡 没有眼泪伴随也不用安慰',
    '自我麻痹也是一种沉醉 only myself不用担心世界粉碎',
    '想听你说你经历 心里留下你印记',
    '如果你是陷阱我会毫无保留跳进去 为你打破墨菲定律',
    '想要深入了解你 就像深海里的鲸 游向海底'
]

export const CALLS = [
    {
        song: '《黄昏日落时》',
        text: '日落的黄昏打在脸上🌇煜公主是个artist💫在忘我地演唱💓爱上也是一种点亮🌟',
        image: 'https://wx4.sinaimg.cn/large/005SF7JFly1hu0zbo4tbsj31xg1xgqv6.jpg',
        info: '',
    },
    {
        song: '《生于未来》',
        text: '钻石💎被切割那面渗透出高级感 💫拉上窗帘也遮盖不住煜皇❤️‍🔥像钻石超级闪✨',
        image: '',
        info: '',
    },
    {
        song: '《没有办法pt2·圣诞限定》',
        text: '',
        image: 'https://wx3.sinaimg.cn/large/005SF7JFly1ht3kp51xppj30u00u0wm7.jpg',
        info: '爱是自由意志的沉沦'
    },
    {
        song: '《煜Freestyle》',
        text: '',
        image: 'https://wx1.sinaimg.cn/large/005SF7JFly1ht3kpdapddj30u00u0tec.jpg',
        info: '“可是我看他真不像假的 你看他哪像真的 眼神”'
    },
    {
        song: '《1+1≠1》',
        text: '曾经💖会畏惧流言蜚语💧但现在变了✨因为骑士会一直守护煜公主👸🏼',
        image: 'https://wx1.sinaimg.cn/large/005SF7JFly1ht3kpcgkooj30ro0ro7br.jpg',
        info: '没有人再逼迫善良一定要战胜邪恶。'
    },
    {
        song: '《摩天轮》',
        text: '当煜公主👸🏼需要骑士的时候 就闪现✈️ 相信爱💞会有魔力🪄它会指引骑士来到煜公主身边🌟',
        image: 'https://wx1.sinaimg.cn/large/005SF7JFly1ht3kp9aeo5j32sb2sb4qq.jpg',
        info: '我想，成熟的爱是爱具体但抽象的人而不是爱完美却抽象的概念'
    },
    {
        song: '《成为你自己好吗》',
        text: '✨过去没有人曾像煜公主👸🏼未来也没有人会像煜公主💫你就是唯一💖的煜公主🌟',
        image: '',
        info: '没有描述'
    },
    {
        song: '《BLUE+PINK》',
        text: '等到煜公主👸🏼想要牵着骑士的手🩵骑士💂带煜公主去环游💞仰望宇宙💫💫',
        image: 'https://wx1.sinaimg.cn/large/005SF7JFly1ht3kp3q7mhj30u00u0n0e.jpg',
        info: '建议单曲循环'
    },
    {
        song: '《清零》',
        text: '煜公主👸🏼流了眼泪💧比鲜花还娇贵🌹一切都清零吧💫喝下这杯用回忆调配🥂',
        image: 'https://wx2.sinaimg.cn/large/005SF7JFly1ht3kpeel7zj30hs0hsadm.jpg',
        info: '听完这首歌，过去所有糟糕的都清零吧'
    },
    {
        song: '《时光河里划着小船摇啊摇》',
        text: '和煜公主👸🏼划小船🛶摇啊摇🌟摇船桨💖摇烦恼✨咧嘴的笑🫧',
        image: 'https://wx1.sinaimg.cn/large/005SF7JFly1ht3kpbdd05j30u00u0wlq.jpg',
        info: '摇四季轮回，我们都变老。'
    },
    {
        song: '《钟》',
        text: '钟🕰️带不走的是牵挂💖的痕迹💧煜公主👸🏼带来的爱❤️藏在时针秒针里',
        image: 'https://wx2.sinaimg.cn/large/005SF7JFly1ht3kpcumgmj30u00u0djm.jpg',
        info: '时间就像一条河卷走你也卷走我'
    },
    {
        song: '《臻！》',
        text: '煜公主的美丽属于骑士✨回忆关于煜公主的👸🏼连同泪滴💧都是粉色💗',
        image: 'https://wx4.sinaimg.cn/large/005SF7JFly1ht3kp4g79uj30u00u0taa.jpg',
        info: '真实没有那么重要，重要的是真实感，所以…..嘘！'
    },
    {
        song: '《普通不普通》',
        text: 'ohoh普通的人生🎞️普通的故事🌠因为煜公主👸🏼而变得不普通🌟',
        image: 'https://wx4.sinaimg.cn/large/005SF7JFly1ht3kp7f9v5j3334334e82.jpg',
        info: '尝试用音乐让自己成为一个表达者——混沌中寻找着什么'
    },
    {
        song: '《between I & U》',
        text: '对煜公主👸🏼骑士想说📢I Like💗I need💓I Love💕u',
        image: 'https://wx4.sinaimg.cn/large/005SF7JFly1ht3kp7f9v5j3334334e82.jpg',
        info: '尝试用音乐让自己成为一个表达者——混沌中寻找着什么'
    },
    {
        song: '《Weekend》',
        text: '喝一杯🍷再喝一杯🥂陪在骑士🫧身边的只有煜公主👸🏼这一位✨',
        image: 'https://wx4.sinaimg.cn/large/005SF7JFly1ht3kp7f9v5j3334334e82.jpg',
        info: '尝试用音乐让自己成为一个表达者——混沌中寻找着什么'
    },
    {
        song: '《B.O.W》',
        text: '',
        image: 'https://wx4.sinaimg.cn/large/005SF7JFly1ht3kp7f9v5j3334334e82.jpg',
        info: '尝试用音乐让自己成为一个表达者——混沌中寻找着什么'
    },
    {
        song: '《WHAT U WANT》',
        text: '煜公主👸🏼和骑士紧紧挨在一起💕 只是呆在原地💫便有面对世界的勇气💖',
        image: 'https://wx4.sinaimg.cn/large/005SF7JFly1ht3kp7f9v5j3334334e82.jpg',
        info: '尝试用音乐让自己成为一个表达者——混沌中寻找着什么'
    },
    {
        song: '《存在》',
        text: '存在✨谁能定义我们的存在⭐️煜公主的灵魂👸🏼会成为骑士们的最爱💖',
        image: 'https://wx4.sinaimg.cn/large/005SF7JFly1ht3kp7f9v5j3334334e82.jpg',
        info: '尝试用音乐让自己成为一个表达者——混沌中寻找着什么'
    },
    {
        song: '《没有办法》',
        text: '💕没有办法✨拿你没有办法💫骑士守护煜公主👸🏼像写童话🫧',
        image: 'https://wx3.sinaimg.cn/large/005SF7JFly1ht3kp42fbdj30pd0pdwg1.jpg',
        info: '落日和你、星辰和你，都合理'
    },
    {
        song: '《嘿，你好》',
        text: '嘿你好📞如果时间有尽头⏳骑士会在那里等煜公主👸🏼不会再错过💓',
        image: 'https://wx3.sinaimg.cn/large/005SF7JFly1ht3kp7samej30kf0kfjth.jpg',
        info: '后来才发现，跟随你一起失去的是我自己。'
    },
    {
        song: '《MY ZONE》',
        text: '为煜公主👸🏼打破墨菲定律🫧煜公主牵动骑士的每一次心跳💓',
        image: 'https://wx4.sinaimg.cn/large/005SF7JFly1ht3kpdmq4ij30u00u0dip.jpg',
        info: 'We1come 2 My Z0N1'
    },
    {
        song: '《CANDY CRUSH》',
        text: '眼睛里的彩虹🌈在燃烧❤️‍🔥身边浩瀚银河🌌在环绕💫骑士会用心听煜公主👸🏼留下的暗号‼️',
        image: 'https://wx4.sinaimg.cn/large/005SF7JFly1ht3kpc0cpcj30u00u0afx.jpg',
        info: '小时候总想长大，长大后又总羡慕别人的小时候。'
    },
    {
        song: '《像朋友一样吗》',
        text: '听过最动听💖的声音🎶是煜公主👸🏼在歌里说爱骑士💘骑士对煜公主的爱💓用语言总无法概括',
        image: 'https://wx3.sinaimg.cn/large/005SF7JFly1ht3kpak9j1j30u00u076l.jpg',
        info: '每个阶段都有终点站，我们得下车往前走，路过学校花店、荒野到海边、城堡和花园，在未来某个节点再会和。'
    },
    {
        song: '《KnowKnow-唯一》',
        text: '对于我们的第一次相遇🌟注定煜公主👸🏼是骑士心中70亿的唯一💫',
        image: 'https://wx2.sinaimg.cn/large/005SF7JFly1ht3kpa5vm8j30n00n0wgq.jpg',
        info: '7even1leveN发了首Remix随便听听吧'
    },
    {
        song: '《WHAT IS LOVE》',
        text: 'Love is a mysterious💖骑士✨只想在煜公主👸🏼心里降落🛬',
        image: 'https://wx2.sinaimg.cn/large/005SF7JFly1ht3kpbn9a1j30n00n0dhu.jpg',
        info: '简介：可以告诉我答案了吗？'
    },
    {
        song: '《狼吟（WPL2020）》',
        text: '骑士眼里只有煜公主 👸🏼永远站边煜公主🌟 女巫有毒🔮猎人有枪⁀➷而你有我💖',
        image: 'https://wx2.sinaimg.cn/large/005SF7JFly1ht3kpdxs8qj30u00u0acu.jpg',
        info: '狼人杀陪我度过了很长一段时间，这首歌写给网易狼人杀，也给同样喜欢玩狼人杀的你们。'
    },
]

export const CALLS_OTHER = [
    {
        song: '《在加纳共和国离婚》',
        text: '✨你还爱我吗💗我还爱你啊💞骑士永远爱着煜公主💖',
        background: 'linear-gradient(89.4deg, rgba(74,77,103,1) -4.3%, rgba(119, 125, 165, 1) 102.1%)',
    },
    {
        song: '《在这个年代找不到浪漫》',
        text: '在这个年代✨找不到浪漫💞因为煜公主快乐👸🏼所以骑士快乐💖',
        background: 'linear-gradient(180.3deg, rgba(221, 221, 221, 1) 5.5%, rgba(110, 136, 161, 1) 90.2%)',
    },
    {
        song: '《第三人称》',
        text: '难过时候不流泪💧流泪也不算伤悲💫对于第三人称的角度而言💌煜只是煜',
        background: 'linear-gradient(225deg, #fef9d7 0%, #d9ded8 10%, #a2b6df 100%)',
    },
    {
        song: '《艾蜜莉》',
        text: '煜公主👸🏼煜公主💌夕阳掉进我心里💖我要带你去寻找💞散落的星星✨',
        background: 'linear-gradient(320deg, #c69df6 0%, #abecd6 0%, #ff9a9e 100%)',
    },
    {
        song: '《11》',
        text: '爱意就像大雨落下🌧️别再叹息皱眉头💖不会忘掉你所有💌会陪煜公主到最后⏳',
        background: 'linear-gradient(260deg, #ebbba7 0%, #fddb92 10%, #6b8cce 100%)',
    },
    {
        song: '《7years》',
        text: "I only see my goals✨I don't believe in failure💖",
        background: 'linear-gradient( 68.4deg,  rgba(248,182,204,1) 0.5%, rgba(192,198,230,1) 49%, rgba(225,246,240,1) 99.8% )',
    },
    {
        song: '《红》',
        text: '✨For tonight ✰⋆º☾骑士为公主做的一切都是应该💕',
        background: 'linear-gradient(225deg, #fbc8d4 0%, #abecd6 10%, #fccb90 100%)',
    },
    {
        song: '《车站》',
        text: '当列车🚄飞奔下一站的爱恨离别💕我仿佛看见车窗外🌃要去向有煜公主的深夜🌙',
        background: 'linear-gradient(to top, #feada6 0%, #f5efef 100%)',
    },
    {
        song: '《Therefore I Am》',
        text: '✨I think, therefore, I am. 💖',
        background: 'linear-gradient(180deg, #a6c1ee 0%, #6b8cce 10%, #fef9d7 100%)',
    },
    {
        song: '《closure》',
        text: 'Maybe we would be closer💞',
        background: 'linear-gradient(60deg, #c1c161 0%, #fccb90 20%, #d9ded8 70%)',
    },
    {
        song: '《忏悔录》',
        text: '我逃出我的身体这感觉很微妙🌈空气中弥漫着爱煜的味道💞彩色的气泡将你我给围绕💫 ',
        background: 'linear-gradient(260deg, #825882 0%, #a194af 0%, #825882 100%)',
    },
    {
        song: '《最长的电影》',
        text: '我们的开始✨是很长的电影🎇放映了百天💌冰上的芭蕾🫧脑海中还在旋转💫望着你💓不会忘记你',
        background: 'linear-gradient(320deg, #d5dee7 0%, #abecd6 0%, #fad0c4 100%)'
    },
    {
        song: '《从前慢》',
        text: '从前的日色变得慢☀车马邮件都慢✉️一生只够爱煜公主一个人💞',
        background: 'linear-gradient(180deg, #e6dee9 0%, #a6c1ee 10%, #d299c2 100%)',
    },
    {
        song: '《coco》',
        text: 'We need the party💞Cococococo✨Cococococo✨',
        background: 'linear-gradient(180deg, #fbc8d4 0%, #f68084 10%, #abecd6 100%)',
    },
    {
        song: '《极恶都市》',
        text: '谁隐藏春秋🍃谁在大雨之后🌧️把旗帜插在最高的楼🌇煜皇掠夺春秋👑毁灭即拯救💫',
        background: 'linear-gradient(360deg, #a18cd1 0%, #feada6 10%, #2b76b9 100%)',
    },
    {
        song: '《会魔法的老人》',
        text: '都会失忆的💘但会有人把他的生命记忆拼凑🌠因为有关煜公主的故事💌我们至今还没听够💓',
        background: 'linear-gradient(to right, #c1c161 0%, #c1c161 0%, #d4d4b1 100%)',
    },
    {
        song: '《爱太痛》',
        text: '爱太痛了💔却无法把你忘了💫能不能不爱了🥀我不能够🌹不能够不爱煜公主💖',
        background: 'linear-gradient(345deg, #fbc8d4 0%, #fef9d7 10%, rgba(119,125,165,1) 100%)',
    },
    {
        song: '《黄色大门》',
        text: '衣柜入面藏着花园🌸心仪男孩长驻于身边💖而心仪的人是煜👸🏼',
        background: 'linear-gradient(300deg, #fef9d7 0%, #d9ded8 10%, #cfc7f8 100%)',
    },
    {
        song: '《Lucid dreams》',
        text: 'I thought you were the one💗Yu is my everything💫',
        background: 'linear-gradient(360deg, #ff9a9e 0%, #fccb90 10%, #2cacd1 100%)',
    },
    {
        song: '《慢冷》',
        text: '慢冷的人啊❄️会自我折磨🦔浪漫让你温柔🍃也让你最惹人泪流💧牵你手💕若无其事牵煜公主的手🌷',
        background: 'linear-gradient(to top, #a8edea 0%, #fed6e3 100%)',
    },
    {
        song: '《卖火柴的小女孩》',
        text: '来上课 听煜教育📕听煜的命令立正稍息💫有人出生就在等待死💘而我出生是在等待成功✨',
        background: 'linear-gradient(150deg, #f68084 0%, #ff9a9e 10%, #c9ffbf 100%)',
    },
    {
        song: '《陈逸伦的解放西》',
        text: '为什么人一定要恋爱呢💕一定是没有遇见过煜公主💫才会假装若无其事清醒着❄️',
        background: 'linear-gradient(120deg, #ebbba7 0%, #c1c161 10%, #2cacd1 100%)',
    },
    {
        song: '《忘了没有》',
        text: '你到底忘了没有忘了没有忘了没有🌟爱煜公主是骑士亲口的承诺💌',
        background: 'linear-gradient(150deg, #d5dee7 0%, #fccb90 10%, #6b8cce 100%)',
    },
    {
        song: '《网易云》',
        text: '希望可以等到煜公主的来到💫希望每次伸手都能得到怀抱💞',
        background: 'linear-gradient(220deg, #a6c1ee 0%, #f5efef 10%, #a2b6df 100%)',
    },
    {
        song: '《了不起》',
        text: '有梦想的人都了不起💫没有理想的人不伤心🌱煜皇会满足所有人对他的期待👑',
        background: 'linear-gradient(260deg, #feada6 0%, #fad0c4 10%, #a2b6df 100%)',
    },
    {
        song: '《偏爱》',
        text: '讲不听也偏要爱💖更努力爱 让你明白💘等你的依赖✨对煜公主偏爱💞',
        background: 'linear-gradient(85deg, #fef9d7 0%, #2b76b9 10%, rgba(110,136,161,1) 100%)',
    },
    {
        song: '《bloody》',
        text: '他不再相信自己生活被宿命主导👣可是他没有目标理想被自动注销🚫还好那只是没做说唱的另一个煜💘',
        background: 'linear-gradient(80deg, #a2b6df 0%, #a2b6df 10%, #ff9a9e 100%)',
    },
    {
        song: '《达尔文》',
        text: '学会认真💫学会忠诚💖适者才能生存🌹和煜公主一起进化成更好的人💞',
        background: 'linear-gradient(180deg, #bedc40 0%, #a2b6df 10%, #fbed96 100%)',
    },
    {
        song: '《记得》',
        text: '谁还记得是谁先说永远的爱我💖公主和骑士手牵手🌟说要一起走到最后💞',
        background: 'linear-gradient(180deg, #a6c0fe 0%, #d5dee7 10%, #abecd6 100%)',
    },
    {
        song: '《暴风雪》',
        text: '暴风雪🌌煜公主带我们走出这冰天里的雪地❄️穿过那封锁线♾️这一首夜曲🎵只属于我们的回忆💖',
        background: 'radial-gradient( circle at top left,  rgba(255,148,148,1) 12.8%, rgba(255,223,155,1) 52.2%)'
    },
    {
        song: '《为你写诗》',
        text: '为你写诗💌为你做不可能的事🫧为你弹奏🎹所有情歌的句子💫我忘了说 最美的是煜👸🏼的名字',
        background: 'linear-gradient(320deg, #fbc8d4 0%, #f68084 0%, #abecd6 100%)',
    },
    {
        song: '《关键词》',
        text: '🍃落叶的位置 谱出一首诗❤️骑士和煜公主的故事🎸才正要开始🎶',
        background: 'linear-gradient(85deg, #fef9d7 0%, #bedc40 10%, #fef9d7 100%)',
    },
    {
        song: '《为什么》',
        text: '✨因为见不到公主👸🏼我思维混乱💔被爱冲昏头脑💧骑士变成笨蛋💌',
        background: 'radial-gradient( circle farthest-corner at 7.5% 24%,  rgba(237,161,193,1) 0%, rgba(250,178,172,1) 25.5%, rgba(190,228,210,1) 62.3%, rgba(215,248,247,1) 93.8% )',
    },
    {
        song: '《镜头》',
        text: '有公主👸🏼这里就是迪士尼✨不是侏罗纪💖把你的镜头对向我🌟',
        background: 'linear-gradient( 109.6deg,  rgba(247,202,201,1) 20.6%, rgba(146,168,209,1) 85.9% )',
    },
    {
        song: '《萱草花》',
        text: '✨遥遥的天之涯💫萱草花开放🌸 煜公主👸🏼就是我牵挂的模样💖',
        background: 'radial-gradient( circle farthest-corner at 10% 20%,  rgba(298,233,132,1) 0%, rgba(247,198,169,1) 54.2%, rgba(21,186,196,1) 100.3% )',
    },
    {
        song: '《怪天气》',
        text: '怪就怪天气☁️像煜公主不讲道理✨那些相爱留下的痕迹💖满街都是骑士溺爱的回忆💫',
        background: 'linear-gradient( 179.1deg,  rgba(247,238,238,1) -1.9%, rgba(247,202,201,1) 44.9%, rgba(145,168,208,1) 96.1% )',
    },
    {
        song: '《刻在我心底的名字》',
        text: '💫刻在骑士心底的名字💖除了煜公主👸🏼还能有谁呢🫧',
        background: 'linear-gradient( 99.6deg,  rgba(112,128,152,1) 10.6%, rgba(242,227,234,1) 32.9%, rgba(234,202,213,1) 52.7%, rgba(220,227,239,1) 72.8%, rgba(185,205,227,1) 81.1%, rgba(154,180,212,1) 102.4% )',
    },
    {
        song: '《奇妙能力歌》',
        text: '我忘了置身濒绝孤岛🐳忘了眼泪不过失效药💊忘了百年无声口号💧不会忘记煜公主👸🏼',
        background: 'radial-gradient( circle farthest-corner at 10% 20%,  rgba(235,131,130,1) 0%, rgba(235,131,130,0.75) 38.6%, rgba(211,177,125,0.52) 72.1%, rgba(211,177,125,0.24) 94.7% )',
    },
    {
        song: '《危险派对》',
        text: '当音乐再次奏响 💫连呼吸都在碰撞❤️‍🔥把心放你手掌🌟沦陷在煜公主的目光👀',
        background: 'linear-gradient(to top, #a8edea 0%, #fed6e3 100%)',
    },
    {
        song: '《空心》',
        text: '热爱💫曾是唯一的信仰💘但现在煜公主👸🏼也是信仰💖',
        background: 'linear-gradient(60deg, rgb(187 205 218) 0%, rgb(103 100 174) 0%, rgb(187 205 218) 100%)',
    },
    {
        song: '《无聊的》',
        text: '或许我🌟太需要一个证明💖没有煜公主👸🏼骑士真的不行‼️',
        background: 'linear-gradient(345deg, #5753C9 0%, #e03838 10%, #9be15d 100%)',
    },
    {
        song: '《向云端》',
        text: '✨向云端☁山那边⛰海里边🌊陪伴在煜公主身边🩵',
        background: 'linear-gradient(to top, #0c3483 0%, #a2b6df 100%, #6b8cce 100%, #a2b6df 100%)',
    },
    {
        song: '《见山》',
        text: 'WARNING⚠前方高能预警❗随便谁来挑战🔥 没人可以夺走煜皇❤‍🔥半壁的江山‼️',
        background: 'linear-gradient(68.4deg, rgba(248, 182, 204, 1) 0.5%, rgba(192, 198, 230, 1) 49%, rgba(225, 246, 240, 1) 99.8%)'
    },
    {
        song: '《连锁反应》',
        text: '是真的沦陷💫是真的迷恋🫧不小心点燃引线💥想和煜公主👸🏼体验各种体验💖',
        background: 'linear-gradient(180deg, #a2b6df 0%, #feada6 0%, #d5dee7 100%)',
    },
    {
        song: '《上学威龙》',
        text: '我小小年纪为什么要承受听公主讲笑话这份痛苦❓煜皇就是傲慢❤‍🔥你能拿我咋办❓',
        background: 'linear-gradient( 108.9deg,  rgba(251,140,0,1) 11.2%, rgba(0,139,155,1) 88.9% )',
    },
    {
        song: '《烟花易冷》',
        text: '烟花易冷🎆人事易分💕缘份落地生根是我们💞听青春🍃迎来笑声💫羡煞许多人',
        background: 'linear-gradient(to top, #d5dee7 0%, #ffafbd 0%, #c9ffbf 100%)',
    },
    {
        song: '《认真的雪》',
        text: '爱的那么认真💫爱的那么认真 💖公主和骑士之间🫧不会听见说不可能💌',
        background: 'linear-gradient(120deg, #ff9a9e 0%, #a2b6df 0%, #fddb92 100%)',
    },
    {
        song: '《离开地球表面》',
        text: '一颗心噗通噗通的狂跳💓一瞬间烦恼烦恼烦恼全忘掉💫',
        background: 'linear-gradient(120deg, rgb(102 141 148) 0%, rgb(102 141 148) 0%, rgb(198 219 216) 100%)',
    },
    {
        song: '《说散就散》',
        text: '干嘛听苦情歌以为多浪漫💫 听煜公主👸🏼唱才能浪漫不被拆散💘 一起共患难💕',
        background: 'linear-gradient(320deg, #e6dee9 0%, #fbc8d4 0%, #bedc40 100%)'
    },
    {
        song: '《我好想你》',
        text: '我好想你✨好想你💗不露痕迹💫装作无所谓🫧深藏在心💖',
        background: 'linear-gradient(to top, #fbc2eb 0%, #a6c1ee 100%)',
    },
    {
        song: '《回到我们的track》',
        text: '💥Yeah we back on🔥煜皇登基版❤️‍🔥煜帝哥哥back on！💥危险! ! ',
        background: 'linear-gradient(320deg, #cfc7f8 0%, #6b8cce 0%, #d5dee7 100%)',
    },
    {
        song: '《花海》',
        text: '不要你离开💞距离隔不开💫情歌被打败💘爱依然存在💌',
        background: 'linear-gradient(45deg, #ff9a9e 0%, #fad0c4 99%, #fad0c4 100%)',
    },
    {
        song: '《阿拉斯加海湾》',
        text: '你最近是否不再失眠啦✨愿世间温情化作一缕风🍃代替我拥抱你🌈上天你别管我先让小煜幸福吧🫧我会祝福他🌟',
        background: 'linear-gradient(120deg, #c9ffbf 0%, #d9ded8 0%, #fad0c4 100%)',
    },
    {
        song: '《唯一》',
        text: '骑士真的爱煜公主👸🏼没人能比拟✨ 证明煜公主是我们的唯一💖',
        background: 'linear-gradient(120deg, #a6c0fe 0%, #f68084 100%)',
    },
    {
        song: '《克卜勒》',
        text: '一闪一闪亮晶晶 ⭐️煜公主永远为骑士放光明✨',
        background: 'linear-gradient(320deg, #d5dee7 0%, #abecd6 0%, #fad0c4 100%)',
    },
    {
        song: '《someone like you 》',
        text: 'someone like you, everyone love you 💕',
        background: 'linear-gradient(345deg, #2b76b9 0%, #d4d4b1 10%, #2b76b9 100%)',
    },
    {
        song: '《一直很安静》',
        text: '给公主👸🏼的爱一直很安静🫧从一开始就下定决心💌',
        background: 'linear-gradient(60deg, #abecd6 0%, #fbed96 100%)',
    },
    {
        song: '《过火》',
        text: '一直都跟随公主👸🏼的感受让你疯🌬️让你去放纵🫧',
        background: 'linear-gradient(60deg, #abecd6 0%, #fbed96 100%)',
    }
]

export const CALLS_COMMON = [
    '煜皇✨rapper 的神👼🏻说唱的王👑',
    '✨星月🌙共绘至死不渝的浪漫💞献给煜公主殿下👸🏼',
    '在线通缉偷心贼💘煜公主殿下👸🏼',
    '物各有主♡💌♡你是我们的煜公主👸🏼',
    '嘘💗是宇宙神明派来唱歌给你听的煜公主殿下👸🏼',
    '啊，我怎么走不动道了？✨哦，原来是煜公主👸🏼驾到啦！',
    '今天吃了果冻✨喝了脉动🌟还是忍不住对煜公主心动！💖',
    '今日限定🩷煜公主全糖去冰🍬',
    '星尘⭐陨石坠落人间🌈，煜公主👸🏼歌声🎶停留耳边👂🏻',
    '💿煜公主就是行走的唱片♬ ✨挥挥荧光棒🌸疯狂撒花🌸',
    '真情永不变💖煜宝天天见🌹 和煜公主一起摇摆哎💃🏻🕺🏻'
]
